...
# run bowtie2
bowtie2 --threads ${threads} -x /home/jmanthey/references/bear -1 ${workdir}/01_cleaned/${basename_array}_R1.fastq.gz -2 ${workdir}/01_cleaned/${basename_array}_R2.fastq.gz -S ${workdir}/01_bam_files/${basename_array}.sam

source activate bcftools

# filter for mapped reads
samtools view -b -f 2 -@ ${threads} -o ${workdir}/01_bam_files/${basename_array}.bam ${workdir}/01_bam_files/${basename_array}.sam
...
bcftools mpileup --skip-indels -C 0 -d 200 --min-MQ 10 --threads ${threads} -f ${refgenome} ${workdir}/01_bam_files/${basename_array}_final.bam | bcftools call -m --threads ${threads} -o ${workdir}/02_vcf/${basename_array}.vcf
